...
# The footer's "report a bug" form sends data-version, which triage renders as
# the build a report came from. public/index.html is committed and served as-is
# — there is no HTML build here to stamp it into — so rather than a literal in
# the markup that goes stale at the next release, the attribute is set by this
# generated shim. Deferred scripts run in document order, so it has run before
# support-footer.js reads the tag.
VERSION="v$(node -p "require('$REPO/package.json').version")"
cat > "$OUT/support-footer-version.js" <<EOF
// GENERATED by scripts/build-assets.sh — do not edit.
var t = document.querySelector('script[src="/support-footer.js"]');
if (t) t.setAttribute('data-version', '$VERSION');
EOF
